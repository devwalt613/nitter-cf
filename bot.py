...
def fetch_tweets(handle):
    ...
    for item in soup.select("div.timeline-item"):
        ...
        for img in item.select("div.attachments img"):
            src = img.get("src", "")
            if src:
                # Nitter's image proxy serves webp by default (via a
                # ?...&format=webp query param, percent-encoded inside the
                # proxy path). GroupMe's upload API doesn't accept webp,
                # so force jpg here instead.
                src = src.replace("format%3Dwebp", "format%3Djpg").replace("format=webp", "format=jpg")
